...
export default function DeleteFeedButton({ sourceId }: DeleteFeedButtonProps) {
    ...
    return (
        <button
            ...
            disabled={isPending}
            className="opacity-0 group-hover:opacity-100 transition-opacity text-[10px] text-foreground hover:bg-foreground hover:text-background px-1 border border-foreground font-mono disabled:opacity-20"
            title="DELETE_SOURCE"
        >
            {isPending ? "..." : "[X]"}
        </button>
